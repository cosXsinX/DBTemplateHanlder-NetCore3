...
namespace DBTemplateHandler.Core.TemplateHandlers.Handlers
{
    public class TableTemplateHandler
    {
        ...
        public static TableTemplateHandler TableDescriptionPOJOToTableTemplateHandler(ITableModel table, AbstractDatabaseDescriptor databaseDescriptor)
        {
            if (table == null) return null;
            IList<IColumnModel> columns = table.Columns;
            List<String> ColumnNameList = new List<String>();
            Dictionary<String, Properties> ColumnNameIndexedColumnPropertiesMap = new Dictionary<String, Properties>();
            if (!(columns == null))
            {
                int currentIndex;
                IColumnModel currentColumn;
                for (currentIndex = 0; currentIndex < columns.Count; currentIndex++)
                {
                    ...
                    currentProperties.setProperty(COLUMN_PROPERTY_IS_LAST_COLUMN, (currentIndex == (columns.Count - 1)).ToString());
                    ColumnNameIndexedColumnPropertiesMap.Add(currentColumn.Name, currentProperties);
                }
            }

            TableTemplateHandler templateHandler =
                    new TableTemplateHandler(table.Name,
