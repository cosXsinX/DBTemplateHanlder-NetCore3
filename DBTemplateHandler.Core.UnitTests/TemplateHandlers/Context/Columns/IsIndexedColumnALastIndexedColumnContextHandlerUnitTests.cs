...
{
    [TestFixture]
    public class IsIndexedColumnALastIndexedColumnContextHandlerUnitTests
    {
        private IsIndexedColumnALastIndexedColumnContextHandler _tested;

        [OneTimeSetUp]
        public void OneTimeSetUp()
        {
            _tested = new IsIndexedColumnALastIndexedColumnContextHandler(TemplateHandlerBuilder.Build(null));
        }
        ...
        {
            Assert.AreEqual("Is replaced by the inner context when the current column is the last column from the iterated indexed column collection", _tested.ContextActionDescription);
        }
        ...
        public void ProcessContextShouldReturnAnEmptyStringWhenThereIsNoIndexedColumnInColumnList()
        {
            ...
            {
                Column = new ColumnModelForTest() { IsIndexed = true },
                Table = new TableModelForTest() { Columns = new List<IColumnModel>() { new ColumnModelForTest() { IsIndexed = false } } },
            };
        ...
        [Test]
        public void ProcessContextShouldReturnAnEmptyStringWhenTheIndexedColumnModelIsNotTheLastIndexedColumnModel()
        {
            var columnModel = new ColumnModelForTest() { IsIndexed = true };
            var databaseContext = new ProcessorDatabaseContext()
            {
                Column = columnModel,
                Table = new TableModelForTest() { Columns = new List<IColumnModel>() { columnModel, new ColumnModelForTest() { Name = "Another Indexed Column", IsIndexed = true } } }
            };
            var actual = _tested.ProcessContext($"{_tested.StartContext}HelloWorld{_tested.EndContext}",databaseContext);
            Assert.AreEqual(string.Empty, actual);
        }
        ...
        [Test]
        public void ProcessContextShouldReturnAnContextStringValueWhenTheIndexedColumnModelIsTheLastIndexedColumnModel()
        {
            var columnModel = new ColumnModelForTest() { IsIndexed = true };
            var databaseContext = new ProcessorDatabaseContext()
            {
                Column = columnModel,
                Table = new TableModelForTest() { Columns = new List<IColumnModel>() { new ColumnModelForTest() { Name = "Another Indexed Column", IsIndexed = true }, columnModel } }
            };
        ...
        [Test]
        public void ProcessContextShouldReturnAnContextStringValueWhenTheIndexedColumnModelIsTheLastIndexedColumnModelButNotTheLastElementOfColumn()
        {
            var columnModel = new ColumnModelForTest() { IsIndexed = true };
            var databaseContext = new ProcessorDatabaseContext()
            {
                Column = columnModel,
                Table = new TableModelForTest() { Columns = new List<IColumnModel>() {new ColumnModelForTest() { Name = "Another Indexed Column", IsIndexed = true } ,
